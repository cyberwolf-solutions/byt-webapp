...
// Make FullCalendar available globally (useful for dynamic views)
window.FullCalendar = {
  Calendar,
  dayGridPlugin,
  timeGridPlugin,
  ...
  function initModeSetting() {
    ...
    if (lightDarkBtn && lightDarkBtn.length) {
      lightDarkBtn[0].addEventListener("click", function (event) {
        ...
        html.hasAttribute("data-sidebar") &&
        html.getAttribute("data-sidebar") == "dark"
          ? setLayoutMode("data-sidebar", "light", "layout-mode-light", html)
          : setLayoutMode("data-sidebar", "dark", "layout-mode-dark", html);
        // Dispatch the resize event on the window object
        window.dispatchEvent(resizeEvent);
      });
    }
